  ...
  <div class="row">
    ...
    <div class="table-responsive small">
      <table class="table table-sm table-hover text-center">
        ...
        <tbody>
          <?php foreach ($box->allData() as $key => $value) { ?>
            <tr>
          ...
          <div class="modal-body">
            <div class="container-fluid" style="padding-top: 5%; padding-bottom: 5%;">
              <div class="form-group p-1">
                ...
                <input type="text" class="form-control" id="name" placeholder="Enter name" name="name" value="<?php echo isset($boxData['name']) ? $boxData['name'] :  '';  ?>">
                <input type="hidden" class="form-control" name="id" value="<?php echo isset($boxData['id']) ? $boxData['id'] :  '';  ?>">
              </div>
              <div class="form-group p-1">
                <label for="email">Namaz - Time Zone :</label>
                <select class="form-select" name="time_zone">
                  <?php foreach ($timezone->getAllWRTNamaz() as $key => $value) {
                    echo "<option ". ( isset($boxData['time_zone_id']) && $value->t_id == $boxData['time_zone_id'] ? "selected" : '' ). " value=".$value->t_id." > ".$value->t_name."</option>";
                  } ?>
                </select>
              </div>
            </div>
